...
### 2026-06-20
- **优化 RSS 链接**：将 RSS feed 中的文章链接从带 hash 的 `/#/post/xxx` 改为干净的 `/post/xxx`，提升 RSS 阅读器和订阅工具的兼容性与体验。

### 2026-06-20
- **移动端目录（TOC）体验优化**：在移动端和普通平板增加浮动目录按钮，点击后弹出底部抽屉式目录，复用现有 TableOfContents 组件。桌面端保持原有右侧悬浮目录不变。
